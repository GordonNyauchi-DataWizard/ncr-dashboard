...
with tab1:
    ...
    if st.session_state.rc_categories is None:
        ...
        if df_training is not None and "Description" in df_training.columns and "Category" in df_training.columns:
            # Use real training data
            descriptions = df_training["Description"].tolist()
            categories = df_training["Category"].tolist()
            st.session_state.rc_centroids = compute_centroids(categories, descriptions)
        else:
            # Fallback to dummy data (category names)
