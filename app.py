        ...
        if sas_url:
            ...
            if df_training is not None:
                ...
                for col in df_training.columns:
                    if col.lower().strip() == "description":
                        desc_col = col
                    if "category" in col.lower() or "root cause" in col.lower():
                        cat_col = col
                
                if desc_col and cat_col:
                    # Use real training data
                    descriptions = df_training[desc_col].tolist()
                    categories = df_training[cat_col].tolist()
                    st.success(f"✅ Loaded {len(df_training)} Root Cause training examples from {desc_col} & {cat_col}")
                    st.session_state.rc_centroids = compute_centroids(categories, descriptions)
                else:
                    st.warning(f"⚠️ Could not find 'Description' and 'Category' columns. Found: {list(df_training.columns)}")
    ...
    if st.session_state.ca_categories is None:
        ...
        if sas_url:
            ...
            if df_training is not None:
                ...
                for col in df_training.columns:
                    if "description" in col.lower() or "action" in col.lower():
                        desc_col = col
                ...
                if desc_col and cat_col:
                    ...
                    descriptions = df_training[desc_col].tolist()
                    categories = df_training[cat_col].tolist()
                    st.success(f"✅ Loaded {len(df_training)} Corrective Action training examples from {desc_col} & {cat_col}")
