...
def load_training_data_rc():
    """Load Root Causes training data from Azure Blob Storage using SAS token."""
    try:
        with st.spinner("Loading Root Causes training data from Azure..."):
            # Use Azure Blob SDK with full SAS URL
            blob_client = BlobClient.from_blob_url(TRAINING_DATA_RC_URL)
...
def load_training_data_ca():
    """Load Corrective Actions training data from Azure Blob Storage using SAS token."""
    try:
        with st.spinner("Loading Corrective Actions training data from Azure..."):
            # Use Azure Blob SDK with full SAS URL
            blob_client = BlobClient.from_blob_url(TRAINING_DATA_CA_URL)
