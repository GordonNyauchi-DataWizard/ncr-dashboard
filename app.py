...
.sel-match { border-left: 4px solid #dc2626; }
.sel-nomatch { border-left: 4px solid #dc2626; }

hr { border: 1px solid #dc2626 !important; opacity: 0.3; }
...
<div class="header-container">
    <p class="header-title">🔍 NCR Classification</p>
    <p class="header-subtitle">Root Causes & Corrective Actions</p>
</div>
""", unsafe_allow_html=True)
...
with tab1:
    ...
    if st.session_state.rc_categories is None:
        ...
        # Try to load real training data from Azure
        ...
        if sas_url:
            ...
            # Check for actual column names in user's file
            if df_training is not None:
                desc_col = None
                cat_col = None
                
                # First, try exact match for known column names
                ...
                if not desc_col or not cat_col:
                    for col in df_training.columns:
                        if not desc_col and col.lower().strip() == "description":
                            desc_col = col
                        if not cat_col and ("category" in col.lower() or "root cause" in col.lower()):
                            cat_col = col
                
                if desc_col and cat_col:
                    # Use real training data - clean it first
                    descriptions = df_training[desc_col].dropna().astype(str).tolist()
                    categories = df_training[cat_col].dropna().astype(str).tolist()
                    
                    # Remove empty strings and mismatched rows
                    valid_pairs = [(d, c) for d, c in zip(descriptions, categories) 
                                   if d.strip() and c.strip()]
                    
                    if valid_pairs:
                        descriptions = [d for d, c in valid_pairs]
                        categories = [c for d, c in valid_pairs]
                        st.success(f"✅ Loaded {len(valid_pairs)} Root Cause training examples from '{desc_col}' & '{cat_col}'")
                        st.session_state.rc_centroids = compute_centroids(categories, descriptions)
                    else:
                        st.warning("⚠️ No valid training data found (all rows empty)")
                    ...
                    st.warning(f"⚠️ Could not find required columns. Found: {list(df_training.columns)}")
                    dummy_rc_descriptions = ROOT_CAUSE_CATEGORIES
                    st.session_state.rc_centroids = compute_centroids(
                        st.session_state.rc_categories,
                        dummy_rc_descriptions
                ...
                st.warning("⚠️ Could not load training data. Using category names as fallback.")
                dummy_rc_descriptions = ROOT_CAUSE_CATEGORIES
                st.session_state.rc_centroids = compute_centroids(
                    st.session_state.rc_categories,
                    dummy_rc_descriptions
            ...
            st.warning("⚠️ TRAINING_DATA_RC_URL not found in Secrets. Using category names.")
            dummy_rc_descriptions = ROOT_CAUSE_CATEGORIES
            st.session_state.rc_centroids = compute_centroids(
                st.session_state.rc_categories,
                dummy_rc_descriptions
    ...
    if st.session_state.ca_categories is None:
        ...
        if sas_url:
            ...
            if df_training is not None:
                ...
                if not desc_col or not cat_col:
                    for col in df_training.columns:
                        if not desc_col and ("description" in col.lower() or "action" in col.lower()):
                            desc_col = col
                        if not cat_col and ("type" in col.lower() or "category" in col.lower()):
                            cat_col = col
                ...
                if desc_col and cat_col:
                    ...
                    if valid_pairs:
                        descriptions = [d for d, c in valid_pairs]
                        categories = [c for d, c in valid_pairs]
                        st.success(f"✅ Loaded {len(valid_pairs)} Corrective Action training examples from '{desc_col}' & '{cat_col}'")
                        st.session_state.ca_centroids = compute_centroids(categories, descriptions)
                    else:
                        st.warning("⚠️ No valid training data found (all rows empty)")
            ...
            st.warning("⚠️ TRAINING_DATA_CA_URL not found in Secrets. Using category names.")
            dummy_ca_descriptions = CORRECTIVE_ACTION_CATEGORIES
            st.session_state.ca_centroids = compute_centroids(
                st.session_state.ca_categories,
                dummy_ca_descriptions
...
with tab3:
    ...
    if not st.session_state.records:
        ...
        # Display records - rename Type to Record Type
