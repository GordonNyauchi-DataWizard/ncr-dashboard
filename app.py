...
def get_training_url():
    try:
        return st.secrets["TRAINING_DATA_URL"]
    except Exception:
        return None
...
def load_df_from_source():
    ...
    if url:
        ...
        r = requests.get(url)
        r.raise_for_status()
        return pd.read_excel(_io.BytesIO(r.content)), "Azure Blob Storage (secure)"
...
def load_model_and_centroids():
    ...
    df_raw, data_source = load_df_from_source()
    if df_raw is not None:
        ...
        df = df[~df["topic_label"].apply(lambda x: bool(NO_MATCH_PAT.match(str(x))) if pd.notna(x) else True)]
        ...
        norms[norms == 0] = 1
        cat_matrix /= norms
        source = f"✅ Trained from **{data_source}** — {len(df):,} rows, {len(cat_names)} categories"
    else:
        cat_names  = ROOT_CAUSE_CATEGORIES
        cat_matrix = model.encode(cat_names, normalize_embeddings=True, show_progress_bar=False)
        source     = "⚠️ Fallback mode — configure TRAINING_DATA_URL in Streamlit secrets"
...
# ─────────────────────────────────────────────────────────────────────────────
    ...
    if not st.session_state.records:
        ...
        try:
            ...
            # Show only pie chart (full width)
            ...
            # Use distinct categorical colors instead of sequential blues
            ...
            fig2 = px.pie(gc, names="Group", values="Count", template=T,
                          color_discrete_sequence=distinct_colors, height=420)
            fig2.update_traces(textposition="inside", textinfo="percent")
            fig2.update_layout(font=FONT, margin=dict(l=0,r=0,t=10,b=10), showlegend=True, 
                               legend=dict(x=1.05, y=1, xanchor="left", yanchor="top", bgcolor="rgba(255,255,255,0)"))
            ...
            try:
                ...
                # Remove DateTime column if it already exists to prevent conflicts
                if "DateTime" in df_copy.columns:
                    df_copy = df_copy.drop(columns=["DateTime"])
                
                # Try to use Timestamp if available
                if "Timestamp" in df_copy.columns and df_copy["Timestamp"].notna().sum() > 0:
                    df_copy["DateTime"] = pd.to_datetime(df_copy["Timestamp"], errors='coerce')
                else:
                    # If no valid timestamps, create sequential times for the day
                    df_copy["DateTime"] = pd.to_datetime("today").normalize() + pd.to_timedelta(range(len(df_copy)), unit='H')
                
                df_trend = df_copy.dropna(subset=["DateTime"])
                
                if len(df_trend) > 0:
                    # Group by date and hour
                    trend = df_trend.groupby([df_trend["DateTime"].dt.date, df_trend["DateTime"].dt.hour]).size().reset_index(name="Count")
                    trend.columns = ["Date", "Hour", "Count"]
                    # Create a display timestamp
                    trend["DateTime"] = pd.to_datetime(trend["Date"].astype(str) + " " + trend["Hour"].astype(str).str.zfill(2) + ":00:00")
                    trend["Count"] = trend["Count"].astype(int)
                    
                    if len(trend) > 0:
                        fig3 = px.line(trend, x="DateTime", y="Count", template=T, height=220, markers=True,
                                       color_discrete_sequence=["#1c3060"])
                        fig3.update_layout(margin=dict(l=0,r=0,t=10,b=10), font=FONT, plot_bgcolor="#f8fafc",
                                          yaxis=dict(tickformat="d"), xaxis=dict(tickformat="%H:%M:%S"))
                        st.plotly_chart(fig3, use_container_width=True)
                    else:
                        st.info("No data available for trend chart.")
                else:
                    st.info("No valid timestamps for trend analysis.")
            except Exception as e:
                st.warning(f"Trend chart error: {str(e)}")
        ...
        # Recalculate all metrics fresh to ensure they're up-to-date
        total = len(df)
        n_ncrs = df["NCR Number"].nunique()
        n_cats = df["Root Cause Category"].nunique()
        matched = (df["Root Cause Category"] != "No Match").sum()
        ...
        # Get most frequent
        most_freq = df[df["Root Cause Category"] != "No Match"]["Root Cause Category"].value_counts().idxmax() if matched > 0 else "N/A"
        
        # Get least frequent - handle ties correctly
        if matched > 0:
            vc = df[df["Root Cause Category"] != "No Match"]["Root Cause Category"].value_counts()
            min_count = vc.min()
            max_count = vc.max()
            
            # If all categories have same count, they're all tied
            if min_count == max_count:
                most_freq = f"All {len(vc)} categories (tied)"
                least_freq = f"All {len(vc)} categories (tied)"
            else:
                # Only get categories with minimum count (not maximum)
                least_freq_list = vc[vc == min_count].index.tolist()
                # Exclude the most frequent category to ensure no overlap
                least_freq_list = [cat for cat in least_freq_list if cat != most_freq]
                
                if len(least_freq_list) > 0:
                    least_freq = least_freq_list[0] if len(least_freq_list) == 1 else f"{len(least_freq_list)} categories (tied)"
                else:
                    least_freq = "N/A"
        else:
            least_freq = "N/A"
