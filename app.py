    ...
    if not st.session_state.records:
        ...
        try:
            ...
            try:
                ...
                # Remove DateTime column if it already exists
                if "DateTime" in df_copy.columns:
                    df_copy = df_copy.drop(columns=["DateTime"])
                
                # Try to use Timestamp if available, otherwise create sequential dates
                if "Timestamp" in df_copy.columns and df_copy["Timestamp"].notna().sum() > 0:
                    ...
                    # Create sequential dates (one record per day)
                    df_copy["DateTime"] = pd.to_datetime("today").normalize() - pd.to_timedelta(range(len(df_copy)-1, -1, -1), unit='D')
                
                if len(df_copy) > 0:
                    # Sort by date and get cumulative count
                    df_sorted = df_copy.sort_values("DateTime").reset_index(drop=True)
                    df_sorted["Date"] = df_sorted["DateTime"].dt.strftime('%Y-%m-%d')  # Format as string YYYY-MM-DD
                    df_sorted["Cumulative Count"] = range(1, len(df_sorted) + 1)
                    
                    trend_data = df_sorted[["Date", "Cumulative Count"]].drop_duplicates(subset=["Date"], keep="last")
                    
                    if len(trend_data) > 0:
                        fig3 = px.line(trend_data, x="Date", y="Cumulative Count", 
                                       template=T, height=220, markers=True,
                                       color_discrete_sequence=["#1c3060"])
                        fig3.update_layout(margin=dict(l=0,r=0,t=10,b=10), font=FONT, 
                                          plot_bgcolor="#f8fafc",
                                          yaxis=dict(tickformat="d", tickmode="linear", dtick=1),
                                          xaxis=dict(type="category"))
                        st.plotly_chart(fig3, use_container_width=True)
                else:
                    st.info("No records to display in trend.")
            except Exception as e:
                st.error(f"NCR Volume Trend Error: {str(e)}")
        ...
        if matched > 0:
            ...
            if min_count == max_count:
                ...
                # Recalculate most frequent from the filtered value counts
                most_freq = vc.idxmax()
                
                # Get categories with minimum count
                least_freq_list = vc[vc == min_count].index.tolist()
                
                if len(least_freq_list) == 1:
                    least_freq = least_freq_list[0]
                elif len(least_freq_list) > 1:
                    least_freq = f"{len(least_freq_list)} categories (tied)"
                else:
                    least_freq = "N/A"
        else:
            least_freq = "N/A"
