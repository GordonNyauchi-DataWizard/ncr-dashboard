...
# ─────────────────────────────────────────────────────────────────────────────
    ...
    if not st.session_state.records:
        ...
        try:
            ...
            try:
                # Simple approach: show cumulative count of records
                if len(df) > 0:
                    # Create a simple sequential index of records
                    trend_data = pd.DataFrame({
                        "Record Number": range(1, len(df) + 1),
                        "Cumulative Count": range(1, len(df) + 1)
                    })
                    
                    fig3 = px.line(trend_data, x="Record Number", y="Cumulative Count", 
