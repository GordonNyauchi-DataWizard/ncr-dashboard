...
with tab1:
    ...
    with col_btn:
        rc_classify_btn = st.button("🔍 Classify & Analyze", key="rc_classify_btn", use_container_width=True, type="primary")
    
    if rc_classify_btn and rc_description.strip():
        # Get suggestions
        ...
        if st.session_state.rc_selected_category:
            if st.session_state.rc_selected_category == "No Match":
                ...
                conf_str = f" &nbsp;|&nbsp; Confidence: {sel_conf:.2f}" if sel_conf else ""
                st.markdown(
...
with tab2:
    ...
    col_btn, col_space = st.columns([1, 4])
    with col_btn:
        ca_classify_btn = st.button("🔍 Classify & Analyze", key="ca_classify_btn", use_container_width=True, type="primary")
    
    if ca_classify_btn and ca_description.strip():
        # Get suggestions
        suggestions = get_top_suggestions(
            ca_description,
