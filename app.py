...
def load_training_data_rc():
    """Load Root Causes training data from Azure Blob Storage using requests."""
    try:
        with st.spinner("Loading Root Causes training data from Azure..."):
            # Use requests to download the file with SAS token
            response = requests.get(TRAINING_DATA_RC_URL, timeout=30)
            response.raise_for_status()  # Raise exception for bad status codes
            
            # Read Excel from downloaded bytes
        ...
        st.error(f"Error downloading Root Causes data: {str(e)}")
        return None
    except Exception as e:
        st.error(f"Error loading Root Causes data: {str(e)}")
        return None

def load_training_data_ca():
    """Load Corrective Actions training data from Azure Blob Storage using requests."""
    try:
        with st.spinner("Loading Corrective Actions training data from Azure..."):
            # Use requests to download the file with SAS token
            response = requests.get(TRAINING_DATA_CA_URL, timeout=30)
            response.raise_for_status()  # Raise exception for bad status codes
        ...
        st.error(f"Error downloading Corrective Actions data: {str(e)}")
        return None
    except Exception as e:
        st.error(f"Error loading Corrective Actions data: {str(e)}")
        return None
